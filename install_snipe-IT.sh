...
sudo sed -i 's/email_host = localhost/email_host = <HOSTNAME>.<DOMAIN>.<TLD>/g' /etc/yum/yum-cron.conf
...
sudo mysql --user=root -e "create user 'snipe_user'@'localhost' identified by 'snipe-itdb';"
...
sudo mkdir /var/www/snipe-it#! /usr/bin/env bash
